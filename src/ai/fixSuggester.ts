...
export class FixSuggester {
  private static FIX_TEMPLATES: Record<string, (issue: Issue) => FixSuggestion | null> = {
  ...
  public static suggest(issues: Issue[]): FixSuggestion[] {
  ...
  public static formatAsMarkdown(suggestions: FixSuggestion[]): string {
    if (suggestions.length === 0) return 'No fix suggestions available.';
