...
import { FRAMEWORKS, BUILD_TOOLS, PACKAGE_MANAGERS } from '../utils/constants.js';

export class Detector {
  public static async detect(projectRoot: string): Promise<ProjectInfo> {
    const packageJson = await readJsonSafe<any>(path.join(projectRoot, 'package.json'));
  ...
  private static async detectPackageManager(projectRoot: string): Promise<any> {
