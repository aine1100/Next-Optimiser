  ...
  public async analyze(project: ProjectInfo): Promise<Issue[]> {
        ...
        for (const file of jsFiles) {
          ...
          if (size > this.config.bundleSizeCritical) {
            issues.push(this.createIssue(
              ...
              `Bundle ${relativePath} is ${formatSize(size)}, exceeding critical limit ${formatSize(this.config.bundleSizeCritical)}.`,
              'critical',
              'bundle',
              relativePath,
              undefined,
              'Implement dynamic imports or split large components into separate chunks.'
            ));
          } else if (size > this.config.bundleSizeWarning) {
            issues.push(this.createIssue(
              'large-size',
              'Large Bundle Warning',
              `Bundle ${relativePath} is ${formatSize(size)}, exceeding warning limit ${formatSize(this.config.bundleSizeWarning)}.`,
              'high',
              'bundle',
              relativePath,
              undefined,
              'Consider using React.lazy or next/dynamic to reduce initial load size.'
            ));
          }
        }
      }
    ...
    if (!foundBundles) {
      issues.push(this.createIssue(
        ...
        'Could not find build directories (.next, dist, build). Run a build first for accurate analysis.',
        'medium',
        'bundle',
        undefined,
        undefined,
        'Run `npm run build` or equivalent before analyzing.'
      ));
    }
