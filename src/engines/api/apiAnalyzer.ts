        ...
        if (expression === 'fetch' || (expression.includes('axios') && !expression.includes('.'))) {
          ...
          if (!tryStatement) {
            issues.push(this.createIssue(
              ...
              `API call via ${expression} is not wrapped in a try-catch block. This can lead to unhandled runtime errors.`,
              'medium',
              'api',
              relativePath,
              call.getStartLineNumber(),
              'Wrap this API call in a try-catch block to handle potential network or server errors.'
            ));
          }
          ...
          if (hook && hook.getExpression().getText() === 'useEffect') {
             ...
             issues.push(this.createIssue(
               'raw-fetch-in-hook',
               'Raw API Call in Hook',
               `Using raw ${expression} inside useEffect can lead to data races and lack of caching.`,
               'medium',
               'api',
               relativePath,
               call.getStartLineNumber(),
               'Consider using a data-fetching library like TanStack Query (React Query) or SWR for automatic caching, deduplication, and error handling.'
             ));
          }
        }
      });
