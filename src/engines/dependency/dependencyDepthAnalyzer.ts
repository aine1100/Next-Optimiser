...
const ESTIMATED_SIZES_KB: Record<string, number> = {
  moment: 290,
  lodash: 70,
  'lodash-es': 25,
  axios: 15,
...
export class DependencyDepthAnalyzer extends AnalysisEngine {
  ...
  public async analyze(project: ProjectInfo): Promise<Issue[]> {
      ...
      if (estimatedKb) {
        ...
        if (estimatedKb > 100) {
          issues.push(
            this.createIssue(
              `large-dep-${pkg}`,
              `Large Dependency: ${pkg} (~${estimatedKb}KB)`,
              `"${pkg}" is estimated at ~${estimatedKb}KB minified. This significantly impacts bundle size.`,
              estimatedKb > 200 ? 'high' : 'medium',
              'dependency',
              'package.json',
              undefined,
              `Consider a lighter alternative or dynamic import for "${pkg}".`
            )
          );
        }
      }
    ...
    if (depCount > 100) {
      issues.push(
        this.createIssue(
          ...
          `Project has ${depCount} dependencies. Large dependency trees increase install time and bundle risk.`,
          'medium',
          'dependency',
          'package.json',
          undefined,
          'Audit dependencies with npm ls or depcheck and remove unused packages.'
        )
      );
    }
    ...
    for (const group of duplicates) {
      issues.push(
        this.createIssue(
          `similar-deps-${group[0]}`,
          `Similar Packages: ${group.join(', ')}`,
          `Found potentially redundant packages: ${group.join(', ')}.`,
          'medium',
          'dependency',
          'package.json',
          undefined,
          'Consolidate to a single package to reduce bundle size.'
        )
      );
    }
    ...
    if (totalEstimatedSize > 1000) {
      issues.push(
        this.createIssue(
          'total-dep-size',
          'High Total Dependency Weight',
          `Estimated heavy dependency weight: ~${(totalEstimatedSize / 1024).toFixed(1)}MB across known large packages.`,
          'high',
          'dependency',
          'package.json',
          undefined,
          'Run a bundle analyzer and replace or lazy-load the largest dependencies.'
        )
      );
    }
  ...
  private findSimilarPackages(deps: Record<string, string>): string[][] {
    const groups: string[][] = [];
    const similarSets = [
    ...
    for (const set of similarSets) {
      ...
      if (found.length > 1) groups.push(found);
    }
