      ...
      sourceFile.getDescendantsOfKind(SyntaxKind.JsxSelfClosingElement).forEach((el) => {
        ...
        if (tag === 'img' && !el.getAttribute('loading')) {
          issues.push(
            this.createIssue(
              'lcp-lazy-loading',
              'Missing Lazy Loading on Image',
              'Image without loading="lazy" may hurt Largest Contentful Paint (LCP).',
              'medium',
              'image',
              relativePath,
              el.getStartLineNumber(),
              'Add loading="lazy" for below-fold images, or use next/image which lazy-loads by default.'
            )
          );
        }
      });
      ...
      sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
        ...
        if (expr === 'document.write' || expr === 'document.writeln') {
          issues.push(
            this.createIssue(
              'cls-document-write',
              'document.write() Hurts CLS',
              'document.write() can cause layout shifts and hurt Cumulative Layout Shift (CLS).',
              'high',
              'render',
              relativePath,
              call.getStartLineNumber(),
              'Avoid document.write(). Use DOM APIs or framework rendering instead.'
            )
          );
        }
      });
      ...
      if (fontImports.length > 2) {
        issues.push(
          this.createIssue(
            ...
            `${fontImports.length} font imports detected. Each can cause layout shift during load.`,
            'medium',
            'render',
            relativePath,
            fontImports[0].getStartLineNumber(),
            'Use next/font to self-host and optimize fonts with zero layout shift.'
          )
        );
      }
              ...
              if (text.includes('height') && !text.includes('minHeight') && text.includes('width')) {
                issues.push(
                  this.createIssue(
                    ...
                    'Potential Layout Shift from Inline Styles',
                    'Fixed width/height without min-height can cause CLS when content loads.',
                    'low',
                    'render',
                    relativePath,
                    attr.getStartLineNumber(),
                    'Reserve space with min-height or aspect-ratio to prevent layout shifts.'
                  )
                );
              }
            }
