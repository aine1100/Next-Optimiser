...
async function runAnalysis(projectRoot, config, correlateRuntime = false) {
...
async function outputReports(result, config, outputPath) {
...
program
    ...
    const checks = [
        ...
        {
            name: 'Next Config',
            status: (await fs.pathExists(path.join(projectRoot, 'next.config.js'))) ||
                (await fs.pathExists(path.join(projectRoot, 'next.config.mjs'))),
        },
        {
            name: 'Build Directory',
            status: (await fs.pathExists(path.join(projectRoot, '.next'))) ||
