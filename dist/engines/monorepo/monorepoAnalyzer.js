...
export class MonorepoAnalyzer extends AnalysisEngine {
    ...
    async analyze(project) {
        ...
        if (packages.length <= 1)
            return issues;
        issues.push(this.createIssue('monorepo-detected', 'Monorepo Detected', `Found ${packages.length} workspace packages. Consider per-package analysis for accurate scoring.`, 'info', 'build', undefined, undefined, 'Run next-optimize analyze in each package directory or use --workspace flag.'));
        const depVersions = new Map();
        for (const pkg of packages) {
            const packageJson = await readJsonSafe(path.join(pkg.path, 'package.json'));
            if (!packageJson)
                continue;
            ...
            if (!hasBuild && packageJson.dependencies?.react) {
                issues.push(this.createIssue(`no-build-${pkg.name}`, `Missing Build Script: ${pkg.name}`, `Workspace package "${pkg.name}" has React but no build script.`, 'medium', 'build', path.join(pkg.path, 'package.json'), undefined, 'Add a build script for production optimization.'));
            }
        }
        for (const [depName, versions] of depVersions) {
            if (versions.size > 1) {
                issues.push(this.createIssue(`duplicate-version-${depName}`, `Duplicate Dependency Versions: ${depName}`, `"${depName}" has ${versions.size} different versions across workspace packages: ${[...versions].join(', ')}.`, 'high', 'dependency', undefined, undefined, 'Align dependency versions across workspace packages to reduce bundle duplication.'));
            }
        }
        return issues;
    }
    async detectWorkspaces(rootPath) {
        const packages = [];
        const rootPkg = await readJsonSafe(path.join(rootPath, 'package.json'));
                    ...
                    if (await fs.pathExists(workspaceDir)) {
                        ...
                        for (const entry of entries) {
                            ...
                            if (await fs.pathExists(path.join(pkgPath, 'package.json'))) {
                                const pkg = await readJsonSafe(path.join(pkgPath, 'package.json'));
        ...
        if (packages.length === 0) {
            const rootPackageJson = await readJsonSafe(path.join(rootPath, 'package.json'));
            if (rootPackageJson) {
