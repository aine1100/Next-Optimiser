    ...
    async analyze(project) {
        ...
        for (const file of imageFiles) {
            ...
            if (size > this.config.imageSizeLimit) {
                issues.push(this.createIssue('large-image', 'Large Image Asset', `Image asset ${relativePath} is ${formatSize(size)}, exceeding limit of ${formatSize(this.config.imageSizeLimit)}.`, 'medium', 'image', relativePath, undefined, 'Compress this image or use a modern format like WebP/AVIF.'));
            }
            if (path.extname(file).match(/\.(png|jpg|jpeg)$/i)) {
                issues.push(this.createIssue('unoptimized-format', 'Unoptimized Image Format', `Image ${relativePath} is using a legacy format. Modern formats like WebP or AVIF offer better compression.`, 'low', 'image', relativePath, undefined, 'Convert this image to WebP or AVIF.'));
            }
        }
            ...
            for (const sourceFile of morphProject.getSourceFiles()) {
                sourceFile.getDescendantsOfKind(SyntaxKind.JsxSelfClosingElement).forEach(el => {
                    if (el.getTagNameNode().getText() === 'img') {
                        issues.push(this.createIssue('legacy-img-tag', 'Legacy <img> Tag Detected', 'Using standard <img> tags in Next.js misses out on automatic image optimization.', 'medium', 'image', path.relative(project.rootPath, sourceFile.getFilePath()), el.getStartLineNumber(), 'Use the next/image component for automatic optimization, lazy loading, and resizing.'));
                    }
                });
            }
        }
