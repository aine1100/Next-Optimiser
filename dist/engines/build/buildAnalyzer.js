    ...
    async analyze(project) {
        ...
        if (packageJson) {
            ...
            if (!packageJson.scripts?.build) {
                issues.push(this.createIssue('missing-build-script', 'Missing Production Build Script', 'A "build" script was not found in package.json. Production builds are essential for performance.', 'high', 'build', 'package.json', undefined, 'Add a "build" script to your package.json (e.g., "next build" or "vite build").'));
            }
        }
        ...
        if (project.framework === 'next') {
            ...
            if (!(await fs.pathExists(nextConfigPath)) && !(await fs.pathExists(nextConfigMjsPath))) {
                issues.push(this.createIssue('missing-next-config', 'Missing next.config.js', 'Next.js configuration file is missing. Default settings might not be optimal for production.', 'low', 'build', undefined, undefined, 'Create a next.config.js file to manage custom optimizations.'));
            }
            else {
