...
export class ConsoleReporter {
    ...
    static getScoreCategory(score) {
        if (score >= SCORING_CATEGORIES.EXCELLENT.min)
            return SCORING_CATEGORIES.EXCELLENT;
        if (score >= SCORING_CATEGORIES.GOOD.min)
            return SCORING_CATEGORIES.GOOD;
        if (score >= SCORING_CATEGORIES.NEEDS_OPTIMIZATION.min)
            return SCORING_CATEGORIES.NEEDS_OPTIMIZATION;
        return SCORING_CATEGORIES.CRITICAL;
    }
    static colorValue(value) {
        if (value >= 90)
    ...
    static reportRecommendations(recommendations) {
